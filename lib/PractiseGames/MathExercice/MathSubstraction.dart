...
class MathSubtractionGame extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MathSubtractionExercise();
  }
}
...
class _MathSubtractionExerciseState extends State<MathSubtractionExercise> {
  ...
  final int maxNumber = 10; // max number to keep it simple
  ...
  void generateNewQuestion() {
    final rand = Random();
    setState(() {
      // To avoid negative answers, ensure firstNumber >= secondNumber
      firstNumber = rand.nextInt(maxNumber) + 1;
      secondNumber = rand.nextInt(firstNumber) + 1; // <= firstNumber
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Center(
          child: Text('Jeu de Soustraction', style: TextStyle(fontWeight: FontWeight.bold)),
        ),
      ),
      body: Stack(
        children: [
